...
counts: §2 requires `Create` to raise `<Aggregate>CreatedEvent`. So a `Manifestation` that can be
created, realized and failed has exactly three, the same shape as `TodoList`'s created /
