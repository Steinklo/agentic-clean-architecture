...
A `TodoList` owns `TodoItem`s and refuses to be archived while any of them is incomplete.
...
A `Manifestation` records a request to make a `TodoItem` true in the physical world, referring to
it by id, and settles once as `Realized` or `Failed`.
...
| Reality adapter | `src/Todo.Infrastructure/Reality/RealityGateway.cs` — the only implementation of `IRealityGateway`; every call returns a failed `Result` |
