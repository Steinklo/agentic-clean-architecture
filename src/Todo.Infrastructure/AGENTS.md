...
`Reality/RealityGateway.cs` — the only implementation of `IRealityGateway`. Every call returns a
failed `Result` carrying the `NotImplemented` category and code `reality.not-implemented`.
