...
| | |
|---|---|
| Aggregate root | `Manifestation` — refers to its `TodoItem` by id, never by navigation |
...
The invariant the aggregate boundary exists for: `Manifestation.Realize()` and `Fail()` both refuse
once the aggregate is already `Realized` or `Failed`.
