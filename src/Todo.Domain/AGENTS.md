...
| | |
|---|---|
| Aggregate root | `Manifestation` — records a request to manifest a `TodoItem`, held by id; settles once |
...
The invariant this aggregate enforces: once `Realize()` or `Fail()` has moved a `Manifestation`
into `Realized` or `Failed`, calling either again refuses with a conflict.
